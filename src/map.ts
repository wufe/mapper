...
export class Map<S, D> implements IMap<S, D>{
	...
	map = (source: S, destination?: D, mapConfiguration?: TConfigurationSetter<ISingleMapConfiguration<S, D>>): D => {
		const conf = mapConfiguration ? mapConfiguration({
			...this._configuration
		} as Configuration<S, D>) : this._configuration;
		return this.internalMap(conf as Configuration<S, D>, source, destination);
	};
	...
	private internalMap(configuration: Configuration<S, D>, source: S, destination?: D) {
		...
		for(let sourceOperation of this._sourceOperations){
			...
			// operationConfiguration.source = source;
			// operationConfiguration.destination = destinationObject;
